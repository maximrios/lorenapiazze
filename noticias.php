...
$sql=mysqli_query("SELECT  ".$lang."_noticias.*, ".$lang."_noticias_imagenes.imagen FROM ".$lang."_noticias INNER JOIN ".$lang."_noticias_imagenes ON  ".$lang."_noticias_imagenes.idr =  ".$lang."_noticias.id WHERE ub1 = 1 GROUP  BY ".$lang."_noticias.id ORDER BY id DESC ");

$total = mysql_num_rows($sql);//Aqui el total de registros
$records_per_page = 3;//Numero de resultado por pagina
$total_pages = ceil($total / $records_per_page);//Total de numeros de paginas
$page = (isset($_GET['page']) && $_GET['page'] <= $total_pages && $_GET['page'] >=1) ? $_GET['page'] : 1;

$offset = ($page - 1) * $records_per_page;//posicion
$limit = " LIMIT ".$offset.", ".$records_per_page;//sql que tenemos que añadir -IMPORTANT-

$sql=mysqli_query("SELECT  ".$lang."_noticias.*, ".$lang."_noticias_imagenes.imagen FROM ".$lang."_noticias INNER JOIN ".$lang."_noticias_imagenes ON  ".$lang."_noticias_imagenes.idr =  ".$lang."_noticias.id WHERE ub1 = 1 GROUP  BY ".$lang."_noticias.id ORDER BY id DESC ".$limit);
              ...
              <?php while ($row=mysql_fetch_assoc($sql)) {     $row["fecha"] = date('d', strtotime($row["fecha"]))." de ".$meses[date('n', strtotime($row["fecha"]))-1]. " del ".date('Y', strtotime($row["fecha"]));  ?>


                        <article>
                            <div class="title"><a href="ver_noticia.php?id=<?=$row["id"]?>"><?=$row["titulo"]?></a></div>
                         <?php if(!empty($row["imagen"])) { ?>  	<div class="imagen"><div class="hover" style="display: none;"><a href="ver_noticia.php?id=<?=$row["id"]?>"><div class="link"></div></a></div><img alt="img1" src="img/<?=$row["imagen"]?>" /></div> <?php } ?>
                            <!--<div class="marc"></div>-->
                            <div class="text"><?=$row["epigrafe"]?></div>
                            <a href="ver_noticia.php?id=<?=$row["id"]?>"><div class="button tipo1">Más Info</div></a>
                       ...
                       if ($page>1) echo '<li><a href="?&page='.($page-1).'">&laquo;</a></li>'; //Anterior
  
                        for ($i = $page; $i <= $total_pages && $i<=($page+$display_pages); $i++) {
                              if ($i == $page) echo '<li class="active"><span class="sr-only">'.$i.'</span></li>'; 
                              else echo  '<li><a href="?&page='.$i.'">'.$i.'</a></li>'; 
                        }
                          

                        if ($page<$total_pages) echo '<li><a href="?&page='.($page+1).'">&raquo;</a></li>'; 
   ...
   <?php  } ?>
            ...
            <?php 
              ...
              $sql=mysqli_query("SELECT  * FROM ".$lang."_noticias  WHERE ub1 != 1 ORDER BY id DESC LIMIT 5  ");
