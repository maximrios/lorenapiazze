<?php 
	...
	if (isset($_POST["enviado"]) && $_POST["enviado"] == $_SESSION["token"]) {
	  ...
	  mysqli_query("UPDATE  galeria SET `nombre`= '".$_POST["nombre"]."', `texto`= '".$_POST["texto"]."', `video`= '".$_POST["video"]."', `categoria`= '".$_POST["categoria"]."'  WHERE id =".$_GET["id"]);
	  ...
	  header("location:".$conf["url"]."?sec=".$sec."&msj=3");
		...
		if(isset($_GET["id"]) && is_numeric($_GET["id"])){
			$sql=mysqli_query("SELECT * FROM galeria WHERE id =".$_GET["id"]);
